...
module.exports = NodeHelper.create({
    ...
    handleData(data) {
	    ...
	    console.log(this.config.keyword);
	    console.log(data);
	    console.log(this.modules);
            if (data.includes(this.config.keyword) || this.listening) {
     		console.log("listening or included");
                this.listening = true;
...
console.log(data);
        ...
        } else if (/(ONE)/g.test(data)) {
            this.sendSocketNotification('ONE');
        } else if (/(TWO)/g.test(data)) {
            this.sendSocketNotification('TWO');
        } else if (/(THREE)/g.test(data)) {
            this.sendSocketNotification('THREE');
        } else if (/(FOUR)/g.test(data)) {
            this.sendSocketNotification('FOUR');
        } else if (/(FIVE)/g.test(data)) {
            this.sendSocketNotification('FIVE');
        } else if (/(SIX)/g.test(data)) {
            this.sendSocketNotification('SIX');
        } else if (/(SEVEN)/g.test(data)) {
            this.sendSocketNotification('SEVEN');
        } else if (/(EIGHT)/g.test(data)) {
            this.sendSocketNotification('EIGHT');
        } else if (/(NINE)/g.test(data)) {
            this.sendSocketNotification('NINE');
        } else if (/(TEN)/g.test(data)) {
            this.sendSocketNotification('TEN');
        ...
        } else if (bytes.r[0].test(data) && bytes.r[1].test(data)) {
